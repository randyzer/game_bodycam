...
describe("theme tokens", () => {
  ...
  it("uses the approved Bodycam dark tactical palette instead of Starter fallback colors", () => {
    const theme = read(themeUrl);

    expect(theme).not.toMatch(/#eee9dc|#ddd6c5|#1f4a3b|#ee4b20|#a92709/i);
    expect(theme).toMatch(/--color-background:\s*#0c0f0e;/);
    expect(theme).toMatch(/--color-surface:\s*#151918;/);
    expect(theme).toMatch(/--color-primary:\s*#8a967f;/);
    expect(theme).toMatch(/--color-accent:\s*#d83a32;/);
  });
...
describe("page-family presentation boundary", () => {
  ...
  it("keeps the unapproved 404 route absent from the Bodycam P0 project", () => {
    const pageInventory = read(pageInventoryUrl);

    expect(existsSync(notFoundPageUrl), `${notFoundPageUrl.pathname} is absent`).toBe(false);
    expect(pageInventory).not.toMatch(/"route":\s*"\/404\.html"/);
    expect(pageInventory).not.toMatch(
      /Coordinate unavailable|signal lost|Return to dispatch/i,
    );
  });

  it("constrains mobile search text and controls to prevent horizontal clipping", () => {
    const globalStyles = read(globalStylesUrl);

    expect(globalStyles).toMatch(
      /@media\s*\(max-width:\s*620px\)[\s\S]*?\.search-shell__header h1\s*\{[^}]*overflow-wrap:\s*anywhere;/,
    );
    expect(globalStyles).toMatch(
      /@media\s*\(max-width:\s*620px\)[\s\S]*?\.search-shell__header > p:last-child\s*\{[^}]*max-width:\s*28ch;[^}]*overflow-wrap:\s*anywhere;/,
    );
    expect(globalStyles).toMatch(
      /@media\s*\(max-width:\s*620px\)[\s\S]*?\.search-console\s*\{[^}]*width:\s*100%;[^}]*max-width:\s*100%;/,
    );
    expect(globalStyles).toMatch(
      /@media\s*\(max-width:\s*620px\)[\s\S]*?\.search-console form > div\s*\{[^}]*grid-template-columns:\s*1fr;/,
    );
    expect(globalStyles).toMatch(
      /@media\s*\(max-width:\s*620px\)[\s\S]*?\.search-console__status,[\s\S]*?\.search-console__note\s*\{[^}]*min-width:\s*0;[^}]*max-width:\s*28ch;[^}]*overflow-wrap:\s*anywhere;/,
    );
  });
});
