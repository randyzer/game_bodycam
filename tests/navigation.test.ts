...
describe("navigation presentation boundary", () => {
  ...
  it("keeps the 390px header menu visible beside the brand", () => {
    const styles = source(globalStylesUrl);
    const narrowStyles = styles.match(/@media \(max-width:\s*620px\)\s*\{([\s\S]*)\n\}/)?.[1] ?? "";
    const headerRule = narrowStyles.match(/\.site-header__inner\s*\{([^}]*)\}/)?.[1];
    const brandRule = narrowStyles.match(/\.brand\s*\{([^}]*)\}/)?.[1];
    const logoRule = narrowStyles.match(/\.brand__logo\s*\{([^}]*)\}/)?.[1];
    const mobileNavRule = narrowStyles.match(/\.mobile-nav\s*\{([^}]*)\}/)?.[1];

    expect(headerRule).toMatch(/gap:\s*\d+px/);
    expect(brandRule).toMatch(/min-width:\s*0/);
    expect(logoRule).toMatch(/width:\s*3[0-9]px/);
    expect(mobileNavRule).toMatch(/flex:\s*0\s+0\s+auto/);
    expect(mobileNavRule).toMatch(/margin-left:\s*auto/);
  });
});
