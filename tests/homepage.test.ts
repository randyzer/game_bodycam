...
describe("homepage presentation", () => {
  ...
  it("renders the approved home trailer mapping in the first viewport hero", async () => {
    const container = await AstroContainer.create();
    const html = await container.renderToString(HomePage);
    const main = html.match(/<main\b[\s\S]*?<\/main>/)?.[0] ?? "";
    const hero = main.match(/<section class="game-hero[\s\S]*?<\/section>/)?.[0] ?? "";
    const index = source(indexUrl);

    expect(index).toContain('data-media-placement="gallery"');
    expect(index).toContain("trailerMedia={homepage.media.trailerMedia}");
    expect(index).not.toMatch(/homepageMedia|remoteImage|mediaSlot|placementName/);
    expect(hero).toContain('data-media-placement="trailer"');
    expect(hero).toContain("youtube-nocookie.com/embed/AQ3DPo2fvSc");
    expect(main).not.toContain('id="home-media"');
    expect(main).not.toMatch(/<img\b/);
  });

  it("renders Bodycam Phase 3 homepage facts without new page machinery", async () => {
    const container = await AstroContainer.create();
    const html = await container.renderToString(HomePage);
    const main = html.match(/<main\b[\s\S]*?<\/main>/)?.[0] ?? "";

    expect(main).toContain("Steam App ID 2406770");
    expect(main).toContain("Reissad Studio");
    expect(main).toContain("Locked &amp; Loaded");
    expect(main).toContain("Windows / Steam");
    expect(source(indexUrl)).toContain("quickFacts:");
    expect(source(indexUrl)).toContain("faq:");
  });

  it("uses the Bodycam authored homepage heading on the brand path", () => {
    const config = source(gameConfigUrl);

    expect(config).toMatch(/displayHeading\s*:\s*"Bodycam Wiki"/);
  });
