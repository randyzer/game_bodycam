...
describe("optional media in existing page shells", () => {
  ...
  it("does not render internal evidence notes in public source sections", async () => {
    const pageWithInternalSourceNote = {
      ...page,
      sources: [
        {
          ...
          evidenceNote: "Internal planning evidence before publication.",
        },
      ],
    };
    const container = await AstroContainer.create();
    const html = await container.renderToString(WikiArticle, {
      props: { page: pageWithInternalSourceNote },
      slots: { default: "<p>Body content remains available.</p>" },
    });

    expect(html).toContain("example.com");
    expect(html).toContain("Checked");
    expect(html).not.toContain("Internal planning evidence before publication.");
  });
  ...
  it("places patch-page trailer media in the article header for early desktop visibility", async () => {
    const patchPage = pageInventory.find(
      (entry) => entry.pageId === "update.locked-loaded",
    )!;
    const container = await AstroContainer.create();
    const html = await container.renderToString(WikiArticle, {
      props: {
      ...
      slots: { default: '<h2 id="start">Start here</h2><p>Patch copy.</p>' },
    });

    const headerIndex = html.indexOf('class="article__header');
    const headerMediaIndex = html.indexOf('data-media-placement="header-trailer"');
    const proseIndex = html.indexOf('class="prose"');

    expect(headerMediaIndex).toBeGreaterThan(headerIndex);
    expect(headerMediaIndex).toBeLessThan(proseIndex);
    expect(html.match(/<iframe\b/g)).toHaveLength(1);
    expect(html).not.toContain('data-media-placement="trailer"');
  });

  it("keeps shared article and media surfaces constrained at 390px", () => {
    const css = readFileSync(new URL("../src/styles/global.css", import.meta.url), "utf8");
    const narrowStyles = css.match(/@media \(max-width:\s*620px\)\s*\{([\s\S]*)\n\}/)?.[1] ?? "";
    const articleTitleRule = narrowStyles.match(/\.article__header h1\s*\{([^}]*)\}/)?.[1];
    const breadcrumbsRule = narrowStyles.match(/\.breadcrumbs ol\s*\{([^}]*)\}/)?.[1];
    const layoutRule = narrowStyles.match(/\.article__layout\s*\{([^}]*)\}/)?.[1];
    const proseRule = narrowStyles.match(/\.article__rail,\s*\n\s*\.prose\s*\{([^}]*)\}/)?.[1];

    expect(articleTitleRule).toMatch(/overflow-wrap:\s*anywhere/);
    expect(articleTitleRule).toMatch(/letter-spacing:\s*0/);
    expect(breadcrumbsRule).toMatch(/flex-wrap:\s*wrap/);
    expect(breadcrumbsRule).toMatch(/min-width:\s*0/);
    expect(layoutRule).toMatch(/grid-template-columns:\s*minmax\(0,\s*1fr\)/);
    expect(proseRule).toMatch(/min-width:\s*0/);
    expect(narrowStyles).toMatch(/\.page-media,\s*\n\s*\.game-media,\s*\n\s*\.video-embed\s*\{[^}]*max-width:\s*100%/);
  });
